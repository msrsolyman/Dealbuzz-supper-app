...
import { Store, Star, ShieldCheck, MessageSquare, Send, Calendar, Edit3, Image as ImageIcon, Package, Clock } from 'lucide-react';
...
export default function SellerProfile() {
  ...
  const [editForm, setEditForm] = useState({ bio: '', coverPhoto: '', profilePicture: '' });
  ...
  const loadData = async () => {
    try {
      ...
      if(foundSeller) {
         setEditForm({ bio: foundSeller.bio || '', coverPhoto: foundSeller.coverPhoto || '', profilePicture: foundSeller.profilePicture || '' });
      }
      setReviews(reviewsRes.data || []);
      
      // Filter products by sellerId
      const sellerProducts = (productsRes.data || []).filter((p: any) => p.sellerId === id);
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 overflow-hidden shadow-sm mb-8 relative">
        ...
        >
          ...
          {isOwner && (
            <button onClick={() => {
              ...
              if(url !== null) { setEditForm({...editForm, coverPhoto: url}); setIsEditingData(true); }
        ...
        <div className="px-8 pb-8 flex flex-col sm:flex-row gap-6 relative z-10">
          <div className="-mt-16 w-32 h-32 bg-white rounded-2xl shadow-xl border-[6px] border-white flex items-center justify-center text-indigo-600 shrink-0 relative overflow-hidden group">
            ...
            {isOwner && (
               <button onClick={() => {
                  const url = prompt('Enter Profile Picture URL:', seller.profilePicture);
                  if(url !== null) { setEditForm({...editForm, profilePicture: url}); setIsEditingData(true); }
          ...
          <div className="pt-4 flex-1">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              ...
              {isEditingData && (
                <button onClick={handleSaveProfile} className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">
                   Save Changes
                </button>
              )}
            </div>
          ...
          <div className="bg-white p-7 rounded-3xl border border-slate-200/60 shadow-sm relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center mb-5 relative z-10">
              ...
              {isOwner && !isEditingData && (
                <button onClick={() => setIsEditingData(true)} className="text-slate-400 hover:text-indigo-600 transition-colors">
            ...
            <div className="relative z-10">
              {isEditingData ? (
                ...
                <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">
                  {seller.bio || "We are a verified merchant on the platform, providing high-quality products and services to our customers. Our commitment is to deliver excellence and ensure customer satisfaction in every transaction."}
                </p>
              )}
            </div>
            <div className="mt-8 pt-6 border-t border-slate-100 space-y-4 font-medium text-sm text-slate-600 relative z-10">
              <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                ...
                <span>Joined <strong>{new Date(seller.createdAt).getFullYear()}</strong></span>
              </div>
            </div>
          </div>
        </div>
          ...
          {activeTab === 'timeline' && (
             <div className="bg-white p-7 md:p-8 rounded-3xl border border-slate-200/60 shadow-sm">
                <h3 className="font-display font-bold text-slate-900 mb-8 text-2xl flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 border border-indigo-100"><Package className="w-5 h-5" /></div>
                   Timeline & Portfolio
                </h3>
                ...
                {products.length === 0 ? (
                   <div className="text-center py-12 text-slate-400">
                      ...
                      <p className="text-sm font-medium">No products listed by this seller yet.</p>
                   </div>
                ) : (
                        ...
                        <div key={product._id} className="relative group">
                           ...
                           <div className="bg-slate-50 border border-slate-200/50 p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                              <div className="flex items-center gap-2 mb-3 text-[10px] uppercase font-black tracking-widest text-slate-400">
                                 <Clock className="w-3.5 h-3.5" /> Added {new Date(product.createdAt).toLocaleDateString()}
                              </div>
                              <div className="flex gap-4">
                                 {product.mainImage ? (
                                    <img src={product.mainImage} alt={product.name} className="w-20 h-20 bg-white rounded-xl object-cover border border-slate-200 shadow-sm" />
                                 ) : (
                                 ...
                                 <div className="flex-1">
                                    ...
                                    <p className="text-sm text-slate-500 line-clamp-2 mb-2">{product.shortDescription || 'No description provided'}</p>
                                    <div className="font-mono font-black text-indigo-600">{formatAmount(product.price)}</div>
                                 </div>
                              </div>
                           </div>
                        </div>
