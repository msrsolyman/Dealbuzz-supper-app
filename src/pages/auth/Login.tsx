...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginType === 'customer' && !['customer', 'super_admin', 'admin'].includes(role)) {
        toast.error('This is the customer portal. You are registered as a seller.');
      } else if (loginType === 'seller' && ['customer'].includes(role)) {
        toast.error('This is the seller portal. You are registered as a customer.');
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 font-sans text-slate-900 border-8 border-slate-900">
      <div className="w-full max-w-sm p-6 bg-white border border-slate-200 shadow-xl rounded-2xl">
        ...
        <p className="text-center text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1 mb-6">
          {loginType === 'customer' ? 'Customer Authentication' : 'Seller Login Panel'}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={loginType === 'customer' ? 'customer@example.com' : 'seller@example.com'}
              required
            />
          </div>
          <div>
          ...
          >
            {loading ? 'Authenticating...' : (loginType === 'customer' ? 'Customer Login' : 'Secure Seller Login')}
          </button>
        </form>
          ...
          {loginType === 'customer' ? (
            ...
            <>
              ...
              <div className="text-center mt-2 flex flex-col gap-2">
                <p className="text-[11px] text-slate-500 font-medium">
                  Want to sell on Dealbuzz? <Link to="/seller-register" className="text-indigo-600 hover:text-indigo-700 font-bold ml-1 hover:underline">Register as Seller</Link>
                </p>
              </div>
            </>
          )}
