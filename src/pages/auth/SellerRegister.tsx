...
export default function SellerRegister() {
  ...
  const handleUserSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Registration successful! Please wait for super admin approval before logging in.');
      navigate('/login');
    } catch (error: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 font-sans text-slate-900 border-8 border-indigo-600 p-4">
      <div className="w-full max-w-md p-8 bg-white border border-slate-200 shadow-xl rounded-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleUserSubmit} className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">I am applying as a...</label>
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: 'product_seller', label: 'Product Seller', icon: Store },
                { id: 'service_seller', label: 'Service Provider', icon: Briefcase },
                ...
                >
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-wider text-center leading-tight">{r.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="pt-2">
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Company/Full Name</label>
            <input
              type="text"
              value={userForm.name}
          ...
          <div>
            ...
            />
             <p className="text-[10px] text-amber-600 mt-2 font-medium">
               Your application will be reviewed by our platform administrators before account activation.
             </p>
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 text-center">
          <p className="text-[11px] text-slate-500 font-medium">
            Already a partner? <Link to="/login" className="text-indigo-600 hover:text-indigo-700 font-bold ml-1 hover:underline">Log in</Link>
          </p>
          <div className="mt-2">
             <Link to="/register" className="text-[10px] text-slate-400 hover:text-slate-600 underline">Register as a customer instead?</Link>
          </div>
        </div>
      </div>
    </div>
