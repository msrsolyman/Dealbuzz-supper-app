...
export default function Accounts() {
  ...
  const canViewBalances = ["super_admin", "admin", "dev"].includes(
    user?.role || "",
  );
  const canExportData = ["super_admin", "admin", "dev"].includes(
    user?.role || "",
  );
  const canAddTransaction = ["super_admin", "admin", "staff", "dev"].includes(
    user?.role || "",
  );
  const canManageSettings = ["super_admin", "admin"].includes(user?.role || "");
  const canViewAmounts = ["super_admin", "admin", "dev"].includes(
    user?.role || "",
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-500 bg-white border border-slate-200 rounded-lg shadow-sm m-4 p-8">
        ...
        <p className="text-center max-w-md">
          Your current role (
          <span className="font-semibold text-slate-700">Customer</span>) does
          not grant permissions to view the tenant's financial accounts and
          ledgers.
        </p>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-2">
            ...
            {!canViewBalances && (
              <span className="ml-2 inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] uppercase tracking-widest font-bold bg-amber-50 text-amber-700 border border-amber-100">
                <Lock className="w-3 h-3" /> Limited Access Mode
              </span>
            )}
          </p>
        </div>
      ...
      {canViewBalances && (
        <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm relative overflow-hidden group">
            ...
            <div className="text-4xl font-display font-bold text-emerald-600 tracking-tight">
              $8,250.00
            </div>
          </div>
          <div className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm relative overflow-hidden group">
            ...
            <div className="text-4xl font-display font-bold text-amber-600 tracking-tight">
              $1,430.00
            </div>
          </div>
        </div>
