      ...
      <div className="flex-1 bg-white border border-slate-200/60 rounded-3xl shadow-sm flex flex-col overflow-hidden min-h-[50vh]">
        <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-4 justify-between bg-slate-50/50">
          <div className="flex gap-2 bg-slate-100/80 p-1 rounded-xl">
            <button onClick={() => setFilterType('all')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${filterType === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>All Items</button>
            <button onClick={() => setFilterType('product')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${filterType === 'product' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/20' : 'text-slate-500 hover:text-slate-700'}`}>Products</button>
            <button onClick={() => setFilterType('service')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${filterType === 'service' ? 'bg-fuchsia-600 text-white shadow-md shadow-fuchsia-500/20' : 'text-slate-500 hover:text-slate-700'}`}>Services</button>
          </div>
              ...
              {filteredCatalog.map(item => (
                ...
                >
                  ...
                  <div className="mt-auto flex items-center justify-between relative z-10">
                    <span className="text-lg font-display font-bold text-slate-900">${(item._type === 'Product' ? item.price : item.rate).toFixed(2)}</span>
      ...
      <div className="w-full md:w-[380px] lg:w-[420px] shrink-0 bg-white border border-slate-200/60 rounded-3xl shadow-sm flex flex-col overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-[#0B1120] text-white flex items-center gap-3 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h3 className="font-display font-bold text-lg leading-tight">Current Sale</h3>
            <div className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mt-0.5">Register #1</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-slate-50/50 flex flex-col custom-scrollbar">
          <div className="mb-4 p-4 bg-white rounded-2xl border border-slate-200/60 shadow-sm grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">Customer</label>
              <select 
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">Warehouse</label>
              <select 
          ...
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-400">
              ...
              <p className="text-xs text-slate-400 mt-1">Select items from catalog to start</p>
            </div>
          ) : (
            <div className="space-y-3">
              {cart.map(item => (
                <div key={item._id} className="bg-white p-3.5 rounded-2xl border border-slate-200/60 shadow-sm flex items-center gap-3 relative overflow-hidden group">
                  ...
                  <div className="font-display font-bold text-slate-900 w-16 text-right">${item.total.toFixed(2)}</div>
        ...
        <div className="border-t border-slate-100 p-6 bg-white space-y-4">
          ...
          <div className="border-t border-slate-200 pt-4 flex items-center justify-between">
            ...
            <span className="text-3xl font-display font-black text-indigo-600 tracking-tight">${grandTotal.toFixed(2)}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-6 pt-2">
            ...
            >
              <Banknote className="w-6 h-6" /> <span className="text-base uppercase tracking-wider">Pay Cash</span>
            </button>
            <button 
              disabled={cart.length === 0 || loading}
              onClick={handleCheckout} 
            ...
            >
              <CreditCard className="w-5 h-5 text-indigo-400" /> <span className="text-sm">Card</span>
            </button>
            <button 
              disabled={cart.length === 0 || loading}
              onClick={handleCheckout} 
            ...
            >
              <Receipt className="w-5 h-5 text-indigo-600" /> <span className="text-sm">Save Draft</span>
            </button>
          </div>
        </div>
      </div>
