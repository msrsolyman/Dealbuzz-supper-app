...
import { Package, Users, FileText, ArrowRight, Search, ShoppingCart, Zap, Sparkles, Shield, Settings, LayoutGrid, Check, X, PlusCircle, Tag, Activity, Clock, AlertTriangle, TrendingUp, Target } from 'lucide-react';
    ...
    const loadData = async () => {
      try {
        if (user?.role === 'customer') {
          ...
          const lowStockItems = rawProducts.filter((p: any) => p.stockCount <= (p.lowStockThreshold || 5));
    ...
    <div className="space-y-6 overflow-hidden font-sans">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <div className="bg-white p-6 md:p-8 border border-slate-200/60 rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
            ...
            <div className="absolute top-6 right-6 opacity-20 group-hover:opacity-40 group-hover:scale-110 transition-all">
               <Package className="w-12 h-12 text-indigo-500" strokeWidth={1.5} />
            </div>
            <div className="text-[10px] text-slate-400 uppercase font-black tracking-[0.2em] mb-4 relative z-10">{t('total_products')}</div>
            <div className="text-5xl font-display font-black text-slate-900 mb-2 relative z-10 tracking-tighter">{stats.products}</div>
            <div className="text-[10px] text-emerald-600 font-bold uppercase tracking-widest relative z-10 bg-emerald-50 inline-block px-2 py-1 rounded">+15 added</div>
          </div>
          
          <div className="bg-white p-6 md:p-8 border border-slate-200/60 rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-fuchsia-50 to-transparent opacity-0 group-hover:opacity-100 rounded-bl-[100px] pointer-events-none transition-opacity duration-300"></div>
            <div className="absolute top-6 right-6 opacity-20 group-hover:opacity-40 group-hover:scale-110 transition-all">
               <FileText className="w-12 h-12 text-fuchsia-500" strokeWidth={1.5} />
            </div>
            <div className="text-[10px] text-slate-400 uppercase font-black tracking-[0.2em] mb-4 relative z-10">{t('total_invoices')}</div>
            <div className="text-5xl font-display font-black text-slate-900 mb-2 relative z-10 tracking-tighter">{stats.invoices}</div>
            <div className="text-[10px] text-fuchsia-600 font-bold uppercase tracking-widest relative z-10 bg-fuchsia-50 inline-block px-2 py-1 rounded">Processing</div>
          </div>
          ...
          <div className="bg-[#0f172a] p-6 md:p-8 border border-slate-800 rounded-[2rem] shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
            ...
            <div className="text-[10px] text-slate-400 uppercase font-black tracking-[0.2em] mb-4 relative z-10">{t('inventory')} (FIFO)</div>
            <div className="text-5xl font-display font-black text-white mb-2 relative z-10 tracking-tighter">{stats.lowStockCount}</div>
            <div className="text-[10px] text-amber-400 font-bold uppercase tracking-widest relative z-10 bg-amber-500/10 border border-amber-500/20 inline-block px-2 py-1 rounded">Items low stock</div>
          </div>
          ...
          <div className="bg-white p-6 md:p-8 border border-slate-200/60 rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
            ...
            <div className="text-[10px] text-slate-400 uppercase font-black tracking-[0.2em] mb-4 relative z-10">{t('active_users')}</div>
            <div className="text-5xl font-display font-black text-slate-900 mb-2 relative z-10 tracking-tighter">1</div>
            <div className="text-[10px] text-sky-600 font-bold uppercase tracking-widest relative z-10 bg-sky-50 inline-block px-2 py-1 rounded">System Online</div>
          </div>
        </div>
      )}
      ...
      {widgets.quickAccess && (
        <div className="bg-white border border-slate-200/60 rounded-[2rem] p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">{t('quick_access')}</h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Fast Navigation</p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <Link to="/invoices" className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 transition-all hover:shadow-md group">
              ...
              <span className="text-sm font-bold leading-tight">{t('new_invoice')}</span>
            </Link>
            <Link to="/products" className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-fuchsia-50 hover:border-fuchsia-200 hover:text-fuchsia-700 transition-all hover:shadow-md group">
              ...
              <span className="text-sm font-bold leading-tight">{t('add_product')}</span>
            </Link>
            <Link to="/pos" className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-200 hover:text-emerald-700 transition-all hover:shadow-md group">
              ...
              <span className="text-sm font-bold leading-tight">{t('launch_pos')}</span>
            </Link>
            <Link to="/services" className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-amber-50 hover:border-amber-200 hover:text-amber-700 transition-all hover:shadow-md group">
              ...
              <span className="text-sm font-bold leading-tight">{t('manage_services')}</span>
            </Link>
          </div>
        </div>
      )}
      ...
      {stats.lowStockItems && stats.lowStockItems.length > 0 && (
        <div className="bg-amber-50 border border-amber-200/60 rounded-3xl p-6 shadow-sm">
           <div className="flex items-center gap-3 mb-4">
             ...
             <div>
               <h3 className="text-sm font-black text-amber-900 uppercase tracking-tight">Low Stock Alert</h3>
               <p className="text-xs font-bold text-amber-700/70 uppercase tracking-widest">{stats.lowStockItems.length} items need restock</p>
             </div>
           </div>
           <div className="flex flex-wrap gap-2">
             {stats.lowStockItems.slice(0, 5).map((item: any) => (
                <div key={item._id} className="px-3 py-2 bg-white border border-amber-200 rounded-lg flex items-center gap-3 shadow-sm">
                   <span className="text-xs font-bold text-slate-700">{item.name}</span>
                   <span className="text-[10px] font-black bg-rose-100 text-rose-600 px-2 py-0.5 rounded">QTY: {item.stockCount}</span>
                </div>
             ))}
             {stats.lowStockItems.length > 5 && (
                <div className="px-3 py-2 bg-amber-100/50 border border-amber-200/50 rounded-lg flex items-center">
                   <span className="text-xs font-bold text-amber-700">+{stats.lowStockItems.length - 5} more</span>
                </div>
             )}
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:h-[450px]">
        {widgets.revenueChart && (
          <div className={`${widgets.recentActivities ? 'lg:col-span-2' : 'lg:col-span-3'} bg-white border border-slate-200/60 rounded-[2.5rem] flex flex-col shadow-sm overflow-hidden`}>
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
              <div>
                <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">{t('revenue_overview')}</h3>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Monthly Analytics</p>
              </div>
              <button className="text-[10px] font-black text-indigo-600 uppercase border border-indigo-200 hover:bg-indigo-50 px-4 py-2 rounded-xl transition-all shadow-sm">Export PDF</button>
            </div>
        ...
        {widgets.recentActivities && (
          <div className={`${widgets.revenueChart ? 'lg:col-span-1' : 'lg:col-span-3'} bg-white border border-slate-200/60 rounded-[2.5rem] flex flex-col shadow-sm overflow-hidden`}>
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Live Audit Feed</p>
              </div>
              <Link to="/audit-logs" className="w-10 h-10 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
            ...
            <div className="flex-1 p-8 space-y-8 overflow-auto custom-scrollbar relative">
                  ...
                  >
                    ...
                    <div className="flex flex-col justify-start pt-1">
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{log.action.replace('_', ' ')}</span>
                        <span className="text-[9px] font-black px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded uppercase tracking-widest">{log.collectionName}</span>
                      </div>
                      <p className="text-xs text-slate-500 font-medium leading-relaxed italic line-clamp-2">By: {log.user?.email || 'System'}</p>
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-2 block">{format(new Date(log.createdAt), 'h:mm a, MMM d')}</span>
                    </div>
                  </motion.div>
                );
              })}
              {stats.logs.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-slate-300 py-20">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.2em]">{t('no_recent_activity')}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-900 to-slate-900 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-12 border border-slate-800">
         ...
         <div className="relative z-10 flex-1">
            ...
            <p className="text-indigo-200 text-sm leading-relaxed max-w-lg font-medium mb-8">
               Our neural network analyzed your last 30 days of transactions. Based on historical velocity and seasonal patterns, here is your restock priority.
            </p>
            <div className="flex gap-4">
               {stats.lowStockItems.slice(0,2).map((item: any) => (
                  <div key={item._id} className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 flex-1">
                     <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center mb-3 text-white">
                        <TrendingUp className="w-4 h-4" />
                     </div>
                     <p className="text-xs text-white/60 uppercase font-black tracking-widest mb-1">High Probability</p>
                     <p className="font-bold text-white truncate" title={item.name}>{item.name}</p>
                     <p className="text-[10px] text-fuchsia-300 font-bold mt-2">Restock +50 units</p>
                  </div>
               ))}
               {stats.lowStockItems.length === 0 && (
                  <p className="text-white">Not enough data to calculate precise forecasting.</p>
               )}
            </div>
         </div>
         <div className="w-full md:w-auto relative z-10">
             <div className="w-48 h-48 border-[16px] border-indigo-500/30 border-t-fuchsia-500 rounded-full animate-[spin_10s_linear_infinite] flex items-center justify-center relative shadow-[0_0_50px_rgba(139,92,246,0.3)]">
                <div className="absolute inset-0 m-auto w-32 h-32 bg-slate-900 rounded-full flex flex-col items-center justify-center border-4 border-slate-800">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 text-center">92% Acc</span>
                </div>
             </div>
         </div>
      </div>
