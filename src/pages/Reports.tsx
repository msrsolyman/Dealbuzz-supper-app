...
import { PieChart as PieChartIcon, TrendingUp, BarChart3, Calendar, FileText, Download, ArrowDown, ArrowUp } from 'lucide-react';
...
export default function Reports() {
  ...
  const COLORS = ['#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#f43f5e', '#6366f1', '#ec4899'];

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-black text-slate-900 uppercase tracking-tight">Business Reports</h1>
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Analytics & Financial Insights</p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white border border-slate-200/60 rounded-[2rem] p-8 shadow-sm">
                  <div className="mb-6 flex justify-between items-end">
                    <div>
                      <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">Monthly Revenue</h3>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Last 6 Months</p>
                    </div>
                  </div>
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={data.monthlyRevenue}>
                        ...
                        <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 10, fontWeight: 700}} width={60} />
                        <Tooltip 
                          cursor={{fill: '#f8fafc'}}
                          contentStyle={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0', borderRadius: '16px', color: '#0f172a', fontSize: '12px', fontWeight: 'bold', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} 
                        />
                ...
                <div className="bg-white border border-slate-200/60 rounded-[2rem] p-8 shadow-sm">
                   <div className="mb-6 flex justify-between items-end">
                    <div>
                      <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">Sales By Category</h3>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">All Time</p>
                    </div>
                  </div>
                  <div className="h-64 flex items-center justify-center">
                    {data.salesByCategory.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={data.salesByCategory} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value">
                      ...
                      <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">No Data Available</div>
                    )}
                  </div>
                </div>
              </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="bg-gradient-to-br from-indigo-500 to-fuchsia-600 rounded-[2rem] p-8 shadow-xl text-white relative overflow-hidden">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-100 mb-2">Total Inventory Value</h3>
                  <div className="text-4xl font-display font-black tracking-tighter">{formatAmount(data.inventoryValuation)}</div>
                </div>
              </div>
              ...
              <div className="bg-white border border-slate-200/60 rounded-[2rem] p-8 shadow-sm">
                 <div className="mb-6 flex justify-between items-end">
                  <div>
                    <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">Valuation By Category</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Current Stock Value</p>
                  </div>
                </div>
                <div className="h-80 flex justify-center">
                  {data.inventoryByCategory.length > 0 ? (
                    ...
                    <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest flex items-center justify-center">No Data Available</div>
                  )}
                </div>
              </div>
            </>
          ...
          {reportType === 'profit' && (
            <>
              <div className="bg-white border border-slate-200/60 rounded-[2rem] p-8 shadow-sm">
                 <div className="mb-6 flex justify-between items-end">
                  <div>
                    <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">Profit & Loss Trend</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Last 6 Months</p>
                  </div>
                </div>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data.profitAndLoss}>
                      ...
                      />
                      <Legend wrapperStyle={{fontSize: '10px', fontWeight: 'bold', paddingTop: '10px'}} />
                      <Line type="monotone" dataKey="revenue" stroke="#3b82f6" strokeWidth={3} dot={{r: 4, fill: '#3b82f6', strokeWidth: 2, stroke: '#ffffff'}} name="Revenue" />
                      <Line type="monotone" dataKey="cost" stroke="#ef4444" strokeWidth={3} dot={{r: 4, fill: '#ef4444', strokeWidth: 2, stroke: '#ffffff'}} name="Cost (Est.)" />
                      <Line type="monotone" dataKey="profit" stroke="#10b981" strokeWidth={3} dot={{r: 4, fill: '#10b981', strokeWidth: 2, stroke: '#ffffff'}} name="Net Profit" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-white border border-slate-200/60 rounded-[2rem] overflow-hidden shadow-sm">
                 <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                  <h3 className="font-display font-black text-slate-900 uppercase tracking-tight">P&L Summary</h3>
                 </div>
                 <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="bg-slate-50 text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">
                          <th className="py-4 px-6 text-left border-b border-slate-100">Period</th>
                          <th className="py-4 px-6 text-right border-b border-slate-100">Revenue</th>
                          <th className="py-4 px-6 text-right border-b border-slate-100">Cost (Est.)</th>
                          <th className="py-4 px-6 text-right border-b border-slate-100">Net Profit</th>
                          <th className="py-4 px-6 text-right border-b border-slate-100">Margin</th>
                        </tr>
                      </thead>
                        ...
                        {data.profitAndLoss.map((row: any, i: number) => {
                          ...
                          return (
                            <tr key={i} className="text-sm hover:bg-slate-50/50 transition-colors">
                              <td className="py-4 px-6 font-bold text-slate-700">{row.month}</td>
                              <td className="py-4 px-6 text-right font-mono font-bold text-sky-600">{formatAmount(row.revenue)}</td>
                              <td className="py-4 px-6 text-right font-mono font-bold text-rose-600">{formatAmount(row.cost)}</td>
                              <td className="py-4 px-6 text-right font-mono font-black text-emerald-600">{formatAmount(row.profit)}</td>
                              <td className="py-4 px-6 text-right font-mono font-bold text-slate-500">{margin.toFixed(1)}%</td>
                            </tr>
                          );
                        })}
                      </tbody>
