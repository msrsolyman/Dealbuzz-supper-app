    ...
    <div className="bg-white border border-slate-200/60 rounded-3xl flex flex-col h-full overflow-hidden shadow-sm font-sans">
      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
        <div className="flex items-center gap-3 text-slate-800 font-bold">
           ...
           <h2 className="text-2xl font-display font-bold tracking-tight">System Audit Logs</h2>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {logs.map((log) => (
              <tr key={log._id} className="hover:bg-slate-50/50 transition-colors group">
                <td className="px-6 py-4 font-mono text-xs text-slate-500 whitespace-nowrap">{format(new Date(log.createdAt), 'MMM d, yyyy h:mm:ss a')}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border 
                    ${log.action === 'CREATE' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 
                      log.action === 'UPDATE' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 
                      log.action === 'DELETE' ? 'bg-rose-50 text-rose-700 border-rose-100' : 
                      'bg-purple-50 text-purple-700 border-purple-100'}`}>
                    {log.action}
                  </span>
                </td>
                <td className="px-6 py-4 font-bold text-slate-900">{log.collectionName}</td>
                <td className="px-6 py-4">
                  <div className="font-mono text-xs font-medium text-slate-900 bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200 inline-block truncate max-w-[200px]" title={log.userId}>{log.userId}</div>
                  <div className="font-mono text-[10px] font-bold text-slate-400 mt-1.5 uppercase tracking-wider">{log.ipAddress}</div>
                </td>
              </tr>
            ))}
            {logs.length === 0 && !loading && (
              <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-500 font-medium">No audit logs found.</td></tr>
            )}
            {loading && <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400">Loading audit logs...</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
