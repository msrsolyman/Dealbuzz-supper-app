...
export default function Products() {
  ...
  const getInitialForm = () => ({
    ...
    approvalStatus: 'pending' // Default for new products
  });
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                      ...
                      {['super_admin', 'admin'].includes(user?.role || '') && (
                        <div className="border-t border-slate-100 pt-4 mt-4">
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 flex items-center gap-1.5"><Shield className="w-3 h-3 text-indigo-500" /> Approval Status</label>
                          <select 
                            name="approvalStatus"
                            value={formData.approvalStatus} 
                            onChange={handleChange}
                            className="w-full border border-slate-200 rounded px-3 py-2 text-sm focus:border-indigo-500 outline-none"
                          >
                            <option value="pending">Pending</option>
                            <option value="approved">Approved</option>
                            <option value="rejected">Rejected</option>
                          </select>
                          <p className="text-[10px] text-slate-400 mt-1">Super Admins can approve products to be published to customers.</p>
                        </div>
                      )}
                    </div>
                  )}
