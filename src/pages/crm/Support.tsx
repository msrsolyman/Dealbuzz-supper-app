                ...
                {selectedTicket.status !== "RESOLVED" &&
                  selectedTicket.status !== "CLOSED" && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-green-100 text-green-700 hover:bg-green-200 text-sm font-bold rounded-xl transition-colors"
                    >
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-slate-50/30">
              ...
              {selectedTicket.messages?.map((msg: any, idx: number) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex gap-4 ${isCustomerMessage ? "" : "flex-row-reverse"}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${isCustomerMessage ? "bg-indigo-100 text-indigo-600" : "bg-fuchsia-100 text-fuchsia-600"}`}
                    >
                      {isCustomerMessage ? "C" : "S"}
                    </div>
                    <div
                      className={`border rounded-2xl p-4 max-w-[80%] shadow-sm ${
                        isCustomerMessage
                          ? "bg-white border-slate-200 rounded-tl-none"
                          : "bg-fuchsia-600 border-fuchsia-700 text-white rounded-tr-none"
                      }`}
                    >
                      <p
                        className={`text-sm ${isCustomerMessage ? "text-slate-700" : "text-fuchsia-50"}`}
                      >
                        {msg.message}
                      </p>
                      <p
                        className={`text-[10px] mt-2 font-medium ${isCustomerMessage ? "text-slate-400" : "text-fuchsia-200"}`}
                      >
                ...
                <div className="p-4 bg-white border-t border-slate-100">
                  <form onSubmit={handleSendMessage} className="flex gap-4">
                    ...
                    <button
                      ...
                      className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition-transform active:scale-95 flex items-center gap-2"
                    >
            ...
            <form onSubmit={handleCreateTicket} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Cannot access invoice"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">
                ...
                <textarea
                  ...
                  className="w-full border p-3 rounded-xl text-sm outline-none focus:border-indigo-500 resize-none"
