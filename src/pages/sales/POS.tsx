...
import { Search, ShoppingCart, Trash2, Plus, Minus, CreditCard, Banknote, DollarSign, Receipt, Package, Zap, Filter, User, Warehouse as WarehouseIcon, Wifi, WifiOff, ScanBarcode } from 'lucide-react';
...
export default function POS() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) {
        if (e.key === 'Enter' && e.target === searchInputRef.current && search) {
          // If hit enter inside search, try to find an exact match
          const match = catalog.find(p => p.sku === search || p.name.toLowerCase() === search.toLowerCase());
          if (match) {
            addToCart(match);
            setSearch('');
          } else {
  ...
  const handleSaveQuote = async () => {
    ...
    setLoading(true);
    try {
      const quotationNumber = 'QT-' + Date.now().toString().slice(-6) + Math.floor(Math.random()*1000);
      const quoteData = {
        quotationNumber,
      ...
      <div className="flex-1 bg-white border border-slate-200/60 rounded-3xl shadow-sm flex flex-col overflow-hidden min-h-[50vh] relative">
        ...
        <AnimatePresence>
          {(isOffline || offlineSyncing) && (
            ...
            >
              {offlineSyncing ? <Wifi className="w-4 h-4 animate-pulse" /> : <WifiOff className="w-4 h-4" />}
              {offlineSyncing ? 'Syncing...' : 'Offline Mode Active - Changes saved locally'}
            </motion.div>
          )}
        </AnimatePresence>

        <div className={`p-5 border-b border-slate-100 flex flex-col xl:flex-row gap-4 justify-between bg-slate-50/50 ${(isOffline || offlineSyncing) ? 'mt-8' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30 custom-scrollbar">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500">
               ...
               <span className="font-bold uppercase tracking-widest text-[10px] text-slate-400">Syncing System...</span>
            </div>
          ) : filteredCatalog.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-300">
              <Package className="w-20 h-20 mb-4 opacity-20" strokeWidth={1} />
              <p className="font-black uppercase tracking-[0.2em] text-xs">Terminal Not Found</p>
              <p className="text-xs mt-2 font-medium">Refine your search parameters</p>
            </div>
          ) : (
            <motion.div 
              layout
                ...
                {filteredCatalog.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-4 relative z-10">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center shadow-sm ${item._type === 'Product' ? 'bg-indigo-50 text-indigo-600' : 'bg-fuchsia-50 text-fuchsia-600'}`}>
                        {item._type === 'Product' ? <Package className="w-4 h-4" /> : <Zap className="w-4 h-4" />}
                      </div>
                      {item._type === 'Product' && (
                         <div className={`flex flex-col items-end`}>
                           <span className={`text-[9px] font-black uppercase tracking-tighter ${item.stockCount > 5 ? 'text-emerald-500' : 'text-rose-500'}`}>
                             {item.stockCount > 0 ? `${item.stockCount} Left` : 'Out of Stock'}
                           </span>
                           <div className={`h-1 w-12 rounded-full mt-1 bg-slate-100 overflow-hidden`}>
                             <div className={`h-full ${item.stockCount > 5 ? 'bg-emerald-400' : 'bg-rose-400'}`} style={{ width: `${Math.min(100, (item.stockCount / 20) * 100)}%` }} />
                           </div>
                         </div>
                      )}
                    </div>

                    <h4 className="font-bold text-slate-800 text-sm mb-1 line-clamp-2 leading-tight relative z-10 group-hover:text-indigo-600 transition-colors">{item.name}</h4>
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest relative z-10 mb-4">{item.category}</div>
                    
                    <div className="mt-auto flex items-end justify-between relative z-10">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 leading-none mb-1">Price</span>
                        <span className="text-xl font-mono font-black text-slate-900 leading-none">{formatAmount(item._type === 'Product' ? item.price : item.rate)}</span>
                      </div>
      ...
      <div className="w-full md:w-[400px] xl:w-[460px] shrink-0 bg-white border border-slate-200/60 rounded-3xl shadow-2xl flex flex-col overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-900 text-white flex items-center gap-4 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h3 className="font-display font-bold text-xl leading-tight">{t('current_sale')}</h3>
            <div className="flex items-center gap-2 mt-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] text-slate-400 uppercase tracking-[0.2em] font-black">Terminal #01 Active</span>
            </div>
          </div>
          <div className="ml-auto flex flex-col items-end relative z-10">
            <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Queue</span>
            <span className="bg-indigo-600 text-white px-3 py-1 rounded-full text-xs font-black shadow-lg shadow-indigo-600/20">{cart.length}</span>
          </div>
        </div>
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200/60 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 mb-1 px-1">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('warehouses')}</span>
              </div>
              <select 
          ...
          <div className="flex-1 min-h-[200px]">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 py-10">
                ...
                <p className="font-black text-[10px] uppercase tracking-widest text-slate-400">{t('cart_is_empty')}</p>
              </div>
            ) : (
              <div className="space-y-3">
                <AnimatePresence initial={false}>
                  {cart.map(item => (
                    ...
                    >
                      <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${item.itemType === 'Product' ? 'bg-indigo-50 text-indigo-500' : 'bg-fuchsia-50 text-fuchsia-500'}`}>
                        {item.itemType === 'Product' ? <Package className="w-5 h-5" /> : <Zap className="w-5 h-5" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-black text-slate-800 text-xs truncate uppercase tracking-tight">{item.name}</h4>
                        <div className="text-[10px] text-slate-400 font-bold mt-1 flex items-center gap-2">
                           <span className="font-mono text-indigo-600">{formatAmount(item.rate)}</span>
                           <span className="w-1 h-1 rounded-full bg-slate-200" />
                           <span className="uppercase tracking-widest">Total: {formatAmount(item.total)}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 bg-slate-50 rounded-xl p-1">
                        <button onClick={() => updateQuantity(item._id, -1)} className="w-6 h-6 rounded-lg bg-white shadow-sm flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-colors">
                          <Minus className="w-3" />
                        </button>
        ...
        <div className="border-t border-slate-100 p-8 bg-white space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 col-span-2">
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Coupon Code</span>
               <div className="flex items-center gap-2">
                 <input 
                  ...
                  />
                  <button onClick={() => {
                        if(couponCode.toUpperCase() === 'VIP20') { setDiscount(20); toast.success('Coupon Applied: 20% off') }
                        else if(couponCode.toUpperCase() === 'WELCOME10') { setDiscount(10); toast.success('Coupon Applied: 10% off') }
                        else { toast.error('Invalid Coupon'); setDiscount(0); }
                  }} className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-[10px] font-black uppercase tracking-widest">Apply</button>
               </div>
            </div>
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">{t('discount_percent')}</span>
               <div className="flex items-center gap-2">
                 <input 
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">{t('tax_percent')}</span>
               <div className="flex items-center gap-2">
                 <input 
                    type="number" min="0" max="100" 
                    value={taxRate} 
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between items-end">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Grand Total Settlement</span>
              <span className="text-slate-400 text-xs font-mono line-through opacity-50">{formatAmount(subtotal)}</span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <motion.div 
                animate={{ opacity: discount > 0 ? 1 : 0, scale: discount > 0 ? 1 : 0.8 }}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            >
              ...
              <span className="text-lg uppercase tracking-widest">{t('pay_cash')}</span>
            </button>
            <button 
              disabled={cart.length === 0 || loading}
              onClick={handleCheckout} 
            ...
            >
              ...
              <span className="text-[10px] uppercase tracking-widest">{t('card')}</span>
            </button>
            <button 
              disabled={cart.length === 0 || loading}
            ...
            >
              ...
              <span className="text-[10px] uppercase tracking-widest">Save Quote</span>
            </button>
            <button 
              disabled={cart.length === 0 || loading}
              onClick={() => setCart([])}
            ...
            >
              ...
              <span className="text-[10px] uppercase tracking-widest">Clear</span>
            </button>
          </div>
        </div>
      </div>
