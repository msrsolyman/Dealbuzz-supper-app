...
import { Store, User, Briefcase, RefreshCw, Settings } from 'lucide-react';
      ...
      <div className="w-full max-w-md p-8 bg-white border border-slate-200 shadow-xl rounded-2xl relative overflow-hidden">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl mb-8">
          <button 
            ...
            className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all ${activeTab === 'user' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Create Account
          </button>
          <button 
        ...
        {activeTab === 'user' ? (
          <form onSubmit={handleUserSubmit} className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">I am registering as a...</label>
            ...
            <div>
              ...
              <input
                ...
                required
              />
              {['product_seller', 'service_seller', 'reseller'].includes(userForm.role) && (
                <p className="text-[10px] text-amber-600 mt-2 font-medium">
                  Note: Seller accounts require administrator approval before logging in.
                </p>
              )}
            </div>
